...
command_cooldown = int(os.environ.get("cavecooldown")
                       )  # in seconds (10800 = 3h)
weekly_cooldown = int(os.environ.get("claimcooldown")
                      )  # in seconds (604800 = 7d)
...
async def on_command_error(ctx, error):
    if isinstance(error, commands.CommandOnCooldown):
        ...
        em = discord.Embed(description=(
            f"**You are still on cooldown** {ctx.message.author.mention}, try again in **{int(days)} days** " + str(time_converted) + "!"), color=(0xff0505))
...
async def cave(ctx):
    ...
    if channel in restricted_channels:
        ...
        selection = preselection[0]
        embed = discord.Embed(description=(
            f"{ctx.message.author.mention} is exploring the BΞASTCAVE!"), color=(0x05ff76))
...
async def claim(ctx):
    ...
    if channel in restricted_channels:
        embed = discord.Embed(description=(
            f"{ctx.message.author.mention} claimed a weekly beastticket!"), color=(0x05ff76))
...
async def ticket(ctx):
    ...
    if channel in restricted_channels:
        getbalance = requests.get(
            f"https://unbelievaboat.com/api/{unbversion}/guilds/{guild_id}/users/{ctx.message.author.id}", headers=headers)
        userbalance = getbalance.json()["cash"]
        if userbalance > 60000:
            payload = {"reason": "Ticket buy", "cash": -60000}
            requests.patch(
                f"https://unbelievaboat.com/api/{unbversion}/guilds/{guild_id}/users/{ctx.message.author.id}", json=payload, headers=headers)
            ...
            embed = discord.Embed(description=(
                f"{ctx.message.author.mention} purchased a BEASTTICKET!"), color=(0x05ff76))
            await ctx.send(embed=embed)
            channel = bot.get_channel(add_money_channel)
            ...
            embed = discord.Embed(description=(
                f"{ctx.message.author.mention} **Not enough <a:Coin:938866055831507034> BEASTCOINS for a BEASTTICKET**"), color=(0xff0505))
            await ctx.send(embed=embed)
            return
    else:
        return
